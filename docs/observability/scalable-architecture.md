...
The [single-node setup](./single-node-setup) handles most workloads, but you need more when:
...
Before diving into components, let's understand the principles guiding this architecture:
...
...the ROI is compelling. Achieving **90%+ cost savings** while maintaining the visibility needed to run reliable production systems is possible.

Start with [Single-Node Setup](./single-node-setup). Scale when you hit real limitations. The architecture grows with you.
...
**Previous**: [← Single-Node Setup](./single-node-setup)
